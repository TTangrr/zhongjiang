...
class Hangye extends Controller

{
	private $object;//����ȫ�ֱ���
	public function _initialize()
	{
		$this->object=model('Hangye');//����model���category����
	}
...
//        }
			...
			return $this->fetch('hangye/index');
		}
		return $this->fetch('');
	}
	...
	public function edit(){
		...
		if(request()->isPost()){
			...
			//halt($res);
			if($res['vaid']){
				$this->success($res['message'],'Hangye/index');exit;
			}else{
	...
	public function find(){
		...
		if(request()->isPost()){
			$data = input('post.inp');
			//  halt($data);
			$index=db('hangye')->where('id|name','like',"{$data}%")->select();
		}
		if($index){
			//    halt("dfgdfg");
